...
(() => {
  ...
  async function loadManagedCourse() {
    ...
    if (!managedCourse) {
      root.innerHTML = '<div class="error-card"><strong>Akun belum terhubung ke mata kuliah.</strong><p>Periksa pemetaan akun pada tabel course_admins di Supabase.</p><a class="button button-primary" href="kelas-admin.html">Ke halaman admin</a></div>';
      return;
    }
    if (groupsError) {
      root.innerHTML = '<div class="error-card"><strong>Fitur kelompok belum disiapkan di database.</strong><p>Jalankan <code>supabase-groups-setup.sql</code>, kemudian muat ulang halaman.</p><a class="button button-primary" href="kelas-admin.html">Ke halaman admin</a></div>';
      return;
    }
    renderPage();
  }
  ...
  async function start() {
    ...
    if (!session) {
      root.innerHTML = '<div class="error-card"><strong>Masuk terlebih dahulu.</strong><p>Halaman pengacak hanya tersedia untuk akun mata kuliah.</p><a class="button button-primary" href="kelas-admin.html">Masuk ke admin</a></div>';
      return;
    }
